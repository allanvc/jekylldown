...
* **The Jekyll theme ecosystem.** First-class support for
  ...
  and minima; any other gem-based theme works with the same pipeline.
* **Publish sources, not build artifacts.** GitHub Pages runs Jekyll
  ...
  repository holds sources only: no `public/` or `_site/` to commit,
  no rendered HTML to keep in sync, publishing is a `git push`.
...
* **A writing loop that stays out of the way.** `serve_site()` runs in
  a background process with incremental rebuilds on save; the console
  stays free.
